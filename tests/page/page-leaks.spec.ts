...
function leakedJSHandles(): string {
  ...
  for (const [h, e] of (globalThis as any).leakedJSHandles) {
    ...
    if (name === '[main] UtilityScript' || name === '[utility] UtilityScript' || name === '[electron] UtilityScript' || name === '[main] InjectedScript' || name === '[utility] InjectedScript' || name === '[electron] ElectronModule' || name === '[worker] UtilityScript' || name === '[worker] ElectronModule')
      continue;
