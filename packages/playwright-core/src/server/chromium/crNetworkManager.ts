...
export class CRNetworkManager {
  ...
  _onRequest(workerFrame: frames.Frame | undefined, requestWillBeSentEvent: Protocol.Network.requestWillBeSentPayload, requestPausedEvent: Protocol.Fetch.requestPausedPayload | null) {
    ...
    // CORS options preflight request is generated by the network stack and does not have the frameId.
    // Retrieve the frame from the original request (initiator).
    const isOptionsPreflight = requestWillBeSentEvent.request.method === 'OPTIONS' && requestWillBeSentEvent.initiator.type === 'preflight';
    if (!frame && isOptionsPreflight) {
      const initiator = this._requestIdToRequest.get(requestWillBeSentEvent.initiator.requestId ?? '');
      frame = initiator?.request._frame;
    }
    ...
    if (requestPausedEvent && isOptionsPreflight && (this._page || this._serviceWorker)!.needsRequestInterception()) {
